import { AxiosError, AxiosResponse } from 'axios';
import {
	getClient,
	ReadDirResult
} from '$lib/socketClient';
const client = getClient();

// export async function read(path: string): Promise<string> {
// 	try {
// 		const request = await requests.get(`/get/database/read`, {
// 			'bot': 'Discord',
// 			'path': path
// 		}, true) as AxiosResponse;

// 		switch (request.status) {
// 			case 200:
// 				break;
// 			case 403:
// 				throw new Error('Forbidden');
// 			case 404:
// 				throw new Error('File not found');
// 			case 500:
// 				throw new Error('Internal server error');
// 			default:
// 				throw new Error(`Unknown error: ${request.status}`);
// 		}

// 		const data = request.data;

// 		// Smart parsing: try to detect if it's JSON, but preserve large numbers as strings
// 		if (typeof data === 'string' && data.trim().length > 0) {
// 			// If it looks like JSON (starts with { or [), try to parse it
// 			const trimmed = data.trim();
// 			if (trimmed.startsWith('{') || trimmed.startsWith('[')) {
// 				try {
// 					return JSON.parse(data);
// 				} catch {
// 					// If parsing fails, return as string
// 					return data;
// 				}
// 			}

// 			// If it's a pure number string that could lose precision, keep as string
// 			if (/^\d+$/.test(trimmed) && trimmed.length > 15) {
// 				return trimmed;
// 			}

// 			// For other strings, return as-is
// 			return data;
// 		}

// 		return String(data);
// 	} catch (error: any) {
// 		if (error instanceof AxiosError) {
// 			if (error.status) {
// 				switch (error.status) {
// 					case 403:
// 						throw new Error('Forbidden');
// 					case 404:
// 						throw new Error('File not found');
// 					case 500:
// 						throw new Error('Internal server error');
// 					default:
// 						throw new Error(`Unknown error: ${error.status}`);
// 				}
// 			}
// 		} else throw error; // If we don't know the error type just leave it for something else to handle
// 	}

// 	return ''; // Just get typescript to shut up, we either get a string or throw an error. This is unreachable
// }

export async function read(path: string): Promise<string | object> {
	const contents = await client.readFile(path);

	if (contents.trim().startsWith('{') || contents.trim().startsWith('[')) {
		try {
			return JSON.parse(contents);
		} catch (e: any) {
			console.error('Error parsing JSON from database read:', e);
		}
	}

	return contents;
}

export async function readDir(path: string, readContents: boolean = false, skipDirectories: boolean = false, recursive: boolean = false): Promise<Array<ReadDirResult>> {
	const result = await client.readDir(path, readContents, recursive);
	if (skipDirectories)
		return result.filter(item => !item.isDirectory);

	return result;
}

export async function write(path: string, data: any): Promise<void> {
	return await client.writeFile(path, typeof data === 'string' ? data : JSON.stringify(data));
}

export async function createDir(path: string): Promise<void> {
	return await client.createDir(path);
}

export async function deleteFile(path: string): Promise<void> {
	return await client.deleteFile(path);
}

export default {
	read,
	readDir,
	write,
	createDir,
	deleteFile
};